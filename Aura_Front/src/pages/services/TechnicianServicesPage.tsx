  ...
  const handleAssociate = async () => {
    ...
    try {
      ...
      if (selectedService.isPlaceholder) {
        const catalogItem = SERVICE_CATALOG.find(item => item.category === selectedService.category);
        const createdService = await ServiceService.create({
          name: catalogItem?.name ?? selectedService.name,
          description: catalogItem?.description ?? selectedService.description,
          category: selectedService.category,
          suggestedPrice: safeRate,
          isActive: true,
        });
